...
void runTest(Board& b, int fromRow, int fromCol, int toRow, int toCol, const string& description, int expected, int& counter);
...
void pinTester(Board& b1){
    ...
    runTest(b1, 1, 3, 1, 7, "Pin Tester: ", 1, counter); 
    runTest(b1, 0, 3, 0, 4, "Make sure King can move: ", 0, counter); 
    runTest(b1, 1, 3, 1, 7, "Pin Tester: ", 0, counter); 
...
void checkmateTester(Board& b1){
    ...
    runTest(b1, 2, 0, 0, 0, "Checkmate Tester: ", 0, counter); 
...
void stalemateTester(Board& b1){
    ...
    runTest(b1, 2, 7, 2, 1, "Stalemate Tester: ", 0, counter); 
    if (b1.isStaleMate("black")) counter++; 
    
    cout << "Stalemate Test: "<< counter / 2 << "/1" << endl; 
}

void runTest(Board& b1, int fromRow, int fromCol, int toRow, int toCol, const string& description, int expected, int& counter){
    int value = b1.movePiece(fromRow, fromCol, toRow, toCol); 

    cout << description << " -> should return " << expected << ": " << value << endl; 
...
void pawnTester(Board& b1){
    ...
    runTest(b1, 1, 0, 2, 0, "Test 1A [Pawn Moves One Square]", 0, counter);

    // TEST 1B: Pawn moves two squares forward
    b1.clearBoard(); b1.placePiece(1, 0, new Pawn("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 1, 0, 3, 0, "Test 1B [Pawn Moves Two Squares]", 0, counter);

    // TEST 1C: Pawn cannot move two squares forward twice
    runTest(b1, 3, 0, 5, 0, "Test 1C [Pawn Moves Two Squares AGAIN!]", 1, counter);
    ...
    runTest(b1, 1, 0, 2, 0, "Test 1D [Pawn Blocked]", 2, counter); 
    ...
    runTest(b1, 1, 0, 2, 1, "Test 1E [Pawn Capture]", 0, counter);

    // TEST 1F: Pawn cannot move diagonally without enemy piece
    runTest(b1, 2, 1, 3, 2, "Test 1F [Illegal Pawn Capture]", 1, counter);
...
void knightTester(Board& b1){
    ...
    for (const auto& move: knightMoves){
        runTest(b1, 3, 3, move.first, move.second, "Test 2A [Check Valid Knight Moves]", 0, counter);
    ...
    runTest(b1, 0, 0, -2, -1, "Test 2B [Out of Bounds Check]", 1, counter);

    runTest(b1, 0, 0, 2, 1, "Test 2B [Out of Bounds Check]", 0, counter);
    ...
    runTest(b1, 0, 0, 2, 1, "Test 2C [Capturing Enemy Pieces]", 0, counter);
    ...
    runTest(b1, 0, 0, 2, 1, "Test 2D [Illegal Friendly Fire]", 2, counter);
    ...
    runTest(b1, 0, 0, 2, 1, "Test 2E [Jumping Knight]", 0, counter);

    // TEST 2F: Illegal Knight Move
    b1.clearBoard(); b1.placePiece(3, 3, new Knight("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 3, 3, 4, 4, "Test 2F [Illegal Knight Move]", 1, counter);
...
void bishopTester(Board& b1){
    ...
    for (const auto& move: bishopMoves){
        runTest(b1, 3, 3, move.first, move.second, "Test 3A [Check Valid Bishop Moves]", 0, counter);
        b1.movePiece(move.first, move.second, 3, 3);
    }

    // TEST 3B: Out of Bounds Check
    b1.clearBoard(); b1.placePiece(0, 0, new Bishop("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, -1, -1, "Test 3B [Out of Bounds Check]", 1, counter);

    runTest(b1, 0, 0, 1, 1, "Test 3B [Out of Bounds Check]", 0, counter);
    ...
    runTest(b1, 0, 0, 7, 7, "Test 3C [Capturing Enemy Pieces]", 0, counter);
    ...
    runTest(b1, 0, 0, 6, 6, "Test 3D [Illegal Friendly Fire]", 2, counter);
    ...
    runTest(b1, 0, 0, 7, 7, "Test 3E [Obstructing Pieces (Same Colour)]", 2, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new Bishop("white")); b1.placePiece(4, 4, new Pawn("black")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 7, 7, "Test 3E [Obstructing Pieces (Opposing Colour)]", 2, counter);

    // TEST 3F: Illegal Bishop Moves
    b1.clearBoard(); b1.placePiece(0, 0, new Bishop("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 7, 0, "Test 3F [Illegal Bishop Moves]", 1, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new Bishop("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 0, 7, "Test 3F [Illegal Bishop Moves]", 1, counter);
...
void rookTester(Board& b1){
    ...
    for (const auto& move: rookMoves){
        runTest(b1, 3, 3, move.first, move.second, "Test 4A [Check Valid Rook Moves]", 0, counter);
        b1.movePiece(move.first, move.second, 3, 3);
    }

    // TEST 4B: Out of Bounds Check
    b1.clearBoard(); b1.placePiece(0, 0, new Rook("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, -7, 0, "Test 4B [Out of Bounds Check]", 1, counter);

    runTest(b1, 0, 0, 3, 0, "Test 4B [Out of Bounds Check]", 0, counter);
    ...
    runTest(b1, 0, 0, 4, 0, "Test 4C [Capturing Enemy Pieces]", 0, counter);
    ...
    runTest(b1, 0, 0, 4, 0, "Test 4D [Illegal Friendly Fire]", 2, counter);
    ...
    runTest(b1, 0, 0, 7, 0, "Test 4E [Obstructing Pieces (Same Colour)]", 2, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new Rook("white")); b1.placePiece(5, 0, new Pawn("black")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 7, 0, "Test 4E [Obstructing Pieces (Opposing Colour)]", 2, counter);


    // TEST 4F: Illegal Rook Moves
    b1.clearBoard(); b1.placePiece(0, 0, new Rook("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 6, 6, "Test 4F [Illegal Rook Moves]", 1, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new Rook("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 1, 2, "Test 4F [Illegal Rook Moves]", 1, counter);
...
void queenTester(Board& b1){
    ...
    for (const auto& move: queenMoves){
        runTest(b1, 3, 3, move.first, move.second, "Test 5A [Check Valid Queen Moves]", 0, counter);
        b1.movePiece(move.first, move.second, 3, 3);
    }
    ...
    runTest(b1, 0, 0, -7, 0, "Test 5B [Out of Bounds Check]", 1, counter);
    runTest(b1, 0, 0, -1, -1, "Test 5B [Out of Bounds Check]", 1, counter);

    runTest(b1, 0, 0, 3, 0, "Test 5B [Out of Bounds Check]", 0, counter);
    b1.movePiece(3, 0, 0, 0); 
    runTest(b1, 0, 0, 1, 1, "Test 5B [Out of Bounds Check]", 0, counter);
    ...
    runTest(b1, 0, 0, 4, 0, "Test 5C [Capturing Enemy Pieces]", 0, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new Queen("white")); b1.placePiece(7, 7, new Pawn("black")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 7, 7, "Test 5C [Capturing Enemy Pieces]", 0, counter);
    ...
    runTest(b1, 0, 0, 4, 0, "Test 5D [Illegal Friendly Fire]", 2, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new Queen("white")); b1.placePiece(6, 6, new Pawn("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 6, 6, "Test 5D [Illegal Friendly Fire]", 2, counter);
    ...
    runTest(b1, 0, 0, 7, 0, "Test 5E [Obstructing Pieces (Same Colour)]", 2, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new Queen("white")); b1.placePiece(5, 0, new Pawn("black")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 7, 0, "Test 5E [Obstructing Pieces (Opposing Colour)]", 2, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new Queen("white")); b1.placePiece(4, 4, new Pawn("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 7, 7, "Test 5E [Obstructing Pieces (Same Colour)]", 2, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new Queen("white")); b1.placePiece(4, 4, new Pawn("black")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 7, 7, "Test 5E [Obstructing Pieces (Opposing Colour)]", 2, counter);

    // TEST 5F: Illegal Queen Moves
    b1.clearBoard(); b1.placePiece(0, 0, new Queen("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 6, 5, "Test 5F [Illegal Queen Moves]", 1, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new Queen("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 1, 2, "Test 5F [Illegal Queen Moves]", 1, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new Queen("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 7, 1, "Test 5F [Illegal Queen Moves]", 1, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new Queen("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 3, 7, "Test 5F [Illegal Queen Moves]", 1, counter);
...
void kingTester(Board& b1){
    ...
    for (const auto& move: kingMoves){
        runTest(b1, 3, 3, move.first, move.second, "Test 6A [Check Valid King Moves]", 0, counter);
        b1.movePiece(move.first, move.second, 3, 3);
    }

    // TEST 6B: Out of Bounds Check
    b1.clearBoard(); b1.placePiece(0, 0, new King("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, -1, -1, "Test 6B [Out of Bounds Check]", 1, counter);

    runTest(b1, 0, 0, 1, 1, "Test 6B [Out of Bounds Check]", 0, counter);
    ...
    runTest(b1, 0, 0, 1, 0, "Test 6C [Capturing Enemy Pieces]", 0, counter);
    ...
    runTest(b1, 0, 0, 1, 0, "Test 6D [Illegal Friendly Fire]", 2, counter);

    // TEST 6E: Illegal King Moves
    b1.clearBoard(); b1.placePiece(0, 0, new King("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 2, 0, "Test 6E [Illegal King Moves]", 1, counter);

    b1.clearBoard(); b1.placePiece(0, 0, new King("white")); b1.printBoard(); cout << endl; 
    runTest(b1, 0, 0, 0, 2, "Test 6E [Illegal King Moves]", 1, counter);
...
void promotionTester(Board& b1){
    ...
    runTest(b1, 6, 3, 7, 3, "Test 7A [Basic Pawn Promotion]", 0, counter); 
    ...
    runTest(b1, 1, 3, 0, 3, "Test 7A [Basic Pawn Promotion]", 0, counter);
    ...
    runTest(b1, 5, 3, 6, 3, "Test 7B [No Promotion BEFORE Last Rank]", 0, counter);
    ...
    runTest(b1, 2, 3, 1, 3, "Test 7B [No Promotion BEFORE Last Rank]", 0, counter);
    ...
    runTest(b1, 6, 3, 7, 4, "Test 7C [Pawn Promotion via Capture]", 0, counter);
    ...
    runTest(b1, 1, 3, 0, 4, "Test 7C [Pawn Promotion via Capture]", 0, counter);
...
void enPassantTester(Board& b1){
    ...
    runTest(b1, 4, 3, 5, 4, "Test 8A [Valid En Passant]", 0, counter);
    ...
    runTest(b1, 4, 3, 5, 4, "Test 8B [Checks for Immediate En Passant]", 1, counter);
    ...
    runTest(b1, 5, 3, 6, 4, "Test 8C [Double Step Required]", 1, counter);
    ...
    runTest(b1, 4, 3, 5, 4, "Test 8D [Adjacency Required]", 1, counter);
    ...
    runTest(b1, 3, 3, 4, 4, "Test 8E [Color Validation]", 1, counter);
...
void castleTester(Board& b1){
    ...
    runTest(b1, 0, 4, 0, 6, "Test 9A [Regular King-Side Castling]", 0, counter);

    // TEST 9B: Regular Queen-Side Castle
    b1.clearBoard(); b1.placePiece(0, 4, new King("white")); b1.placePiece(0, 0, new Rook("white")); 
    b1.printBoard(); cout << endl; 

    runTest(b1, 0, 4, 0, 2, "Test 9B [Regular Queen-Side Castling]", 0, counter);
    ...
    runTest(b1, 0, 4, 0, 6, "Test 9C [Illegal Castling When In Check]", 1, counter);

    // TEST 9D: Illegal Castling Through Pieces
    b1.clearBoard(); b1.placePiece(0, 4, new King("white")); b1.placePiece(0, 7, new Rook("white")); b1.placePiece(0, 6, new Knight("white"));
    b1.printBoard(); cout << endl; 

    runTest(b1, 0, 4, 0, 6, "Test 9D [Illegal Castling Through Pieces]", 2, counter);
    ...
    runTest(b1, 0, 4, 0, 6, "Test 9E [Illegal Castling Through Squares Under Attack]", 1, counter);
    ...
    runTest(b1, 0, 4, 0, 6, "Test 9F [Illegal Castling After King AND/OR Rook Moving]", 1, counter);
    ...
    runTest(b1, 0, 4, 0, 2, "Test 9F [Illegal Castling After King AND/OR Rook Moving]", 1, counter);
